...
std::string filename =
    "TetMesh/fleece.mesh"; // 50k is plenty for nice dynamics and no debris
...
// T suggested_dt = evaluateTimestepLinearElasticityAnalysis(Youngs, nu, rho,
// sim.dx, sim.cfl); if (sim.symplectic) { ZIRAN_ASSERT(sim.step.max_dt <=
...
if (Python::isotropic) {
  std::string path("output/3D_CheesePull/3D_CheesePull_Isotropic_Youngs" +
                   E_str + "_percent" + percent_str + "_eta" + eta_str + "_dx" +
                   dx_str + "_fiberScale" + scale_str + "_residual" + res_str);
  sim.output_dir.path = path;
} else {
  std::string path("output/3D_CheesePull/3D_CheesePull_Youngs" + E_str +
                   "_percent" + percent_str + "_eta" + eta_str + "_dx" +
                   dx_str + "_fiberScale" + scale_str + "_residual" + res_str +
                   "_slackANDholdAndRelease_andBetterSpacedCuts");
  sim.output_dir.path = path;
}
...
if (!Python::isotropic) {
  model.setExtraFiberStiffness(
      0, Python::fiberScale); // only scale elasticity if anisotropic!
}
...
AnalyticCollisionObject<T, dim>
    ground_object(ground_ls, AnalyticCollisionObject<T, dim>::SEPARATE);
...
AnalyticCollisionObject<T, dim>
    spine_object(spineLS, AnalyticCollisionObject<T, dim>::STICKY);
init_helper.addAnalyticCollisionObject(spine_object);

T cutTheta = 26.57; // 26.57 is BEST!! -- degrees down from x-axis that we will
                    // pull the cheese
TV cutDir1(1.0, std::tan(cutTheta *(M_PI / 180.0)),
           0); // this gives a sense of how much down and outward each cut
               // should move based on theta
cutDir1.normalize();
...
// StdVector<T> cutList = { 0, 90, 180, 270, 30, 120, 210, 300 }; // 60, 150,
// 240, 330 }; //IN DEGREES!!!! this list determines the rotation of the cutter
// around the cheese, startCenter is considered theta = 0
...
T slackDuration =
    0.5; // 0.5, how long to give the strand slack before releasing it
T holdDuration = 0.5; // 0.25 hold
StdVector<T> startTimes = {
    0,        0,        0,       0, cutDelay,
    cutDelay, cutDelay, cutDelay}; // cutDelay * 2, cutDelay * 2, cutDelay * 2,
                                   // cutDelay * 2 };
...
for (unsigned int i = 0; i < cutList.size(); i++) {
  ...
  T rotateTheta =
      cutList[i] * (M_PI / 180.0); // now we specifically define each cut!
  TM rotateAroundYAxis;
  rotateAroundYAxis << std::cos(rotateTheta), 0, -1 * std::sin(rotateTheta), 0,
      1, 0, std::sin(rotateTheta), 0, std::cos(rotateTheta);
  ...
  auto sphereTransform = [=](T time, AnalyticCollisionObject<T, dim> &object) {
    ...
    if (time < startTime) { // don't move yet
      ...
      TV translation =
          TV(x0 + (xVel * (time - startTime)), y0 + (yVel * (time - startTime)),
             z0 + (zVel * (time - startTime)));
      TV translation_velocity(xVel, yVel, zVel);
      object.setTranslation(translation, translation_velocity);
    } else if (time < slackTime) { // slacken the strands before releasing
      TV translation =
          TV(x0 + (xVel * (endTime - startTime)) -
                 ((time - endTime) * (xVel * 0.5)),
             y0 + (yVel * (endTime - startTime)) -
                 ((time - endTime) * (yVel * 0.5)),
             z0 + (zVel * (endTime - startTime)) -
                 ((time - endTime) *
                  (zVel * 0.5))); // move slightly in towards cheese
      TV translation_velocity(-1 * xVel * 0.5, -1 * yVel * 0.5,
                              -1 * zVel * 0.5);
      object.setTranslation(translation, translation_velocity);
    } else if (time < holdTime) {
      TV translation =
          TV(x0 + (xVel * (endTime - startTime)) -
                 ((slackTime - endTime) * (xVel * 0.5)),
             y0 + (yVel * (endTime - startTime)) -
                 ((slackTime - endTime) * (yVel * 0.5)),
             z0 + (zVel * (endTime - startTime)) -
                 ((slackTime - endTime) *
                  (zVel * 0.5)));       // move slightly in towards cheese
     ...
     *      }
     *  else if (time < 0.8) {
     *      translation  =TV(0.5 * vx - (t-0.5)* vx/3, 0.5 * vy, 0.5 *vz -
     * (t-0.5)*vz/3)
     *  }
     *  else {
  ...
  // Sphere<T, dim> sphere(TV(10,10,10), sphereRadius); //start all cutters far
  // from cheese
  AnalyticCollisionObject<T, dim> sphereObject(
      sphereTransform, sphere, AnalyticCollisionObject<T, dim>::STICKY);
