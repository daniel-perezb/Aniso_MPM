...
sim.gravity = -.00003 * TV::Unit(1);        // no gravity
...
// //orthotropic and trans iso use full RPIC sim.apic_rpic_ratio = 0; //full
// RPIC
...
std::string filename = "TetMesh/fleece_2_500k.mesh"; // 50k is plenty for nice
                                                     // dynamics and no debris
...
a_1[0] = 0;
a_1[1] = 0;
a_1[2] = 1; // Fibres going straight up
a_1.normalize();
a_0.push_back(a_1);
alphas.push_back(-1);
...
// if (Python::isotropic) {
//   std::string path("output/3D_Fleece/3D_Pork_Isotropic_Youngs" + E_str +
//                    "_percent" + percent_str + "_eta" + eta_str + "_dx" +
//                    dx_str + "_fiberScale" + scale_str);
//   sim.output_dir.path = path;
// } else if (orthotropic) {
//   std::string path("output/3D_Fleece/3D_Pork_Orthotropic_Youngs" + E_str +
//                    "_percent" + percent_str + "_eta" + eta_str + "_dx" +
//                    dx_str + "_fiberScale" + scale_str);
//   sim.output_dir.path = path;
...
// } else {
//   std::string path("output/3D_Fleece/3D_Pork_TransverseIsotropic_Youngs" +
//   E_str +
//                    "_percent" + percent_str + "_eta" + eta_str + "_dx" +
//                    dx_str + "_fiberScale" + scale_str);
//   sim.output_dir.path = path;
...
std::string path("output/3D_Fleece/Fleece_Orthotropic");
sim.output_dir.path = path;
model.setExtraFiberStiffness(
    0, Python::fiberScale); // only scale elasticity if anisotropic!
model.setExtraFiberStiffness(1, Python::fiberScale);
...
TV boxMin(2 - (xSize / 2.0), 2 - (ySize / 2.0) - materialToHold,
          2 - (zSize /
               2.0)); // needs extra material at bottom to hold onto for mode 1
TV boxMax(2 + (xSize / 2.0), 2 + (ySize / 2.0), 2.1 + (zSize / 2.0));
// AxisAlignedAnalyticBox<T, dim> boxLevelSet(boxMin, boxMax);

// Sample particles in the desired level set
// int ppc = 7;
// MpmParticleHandleBase<T, dim> particles_handle =
// init_helper.sampleInAnalyticLevelSet(boxLevelSet, rho, ppc);
...
AnalyticCollisionObject<T, dim>
    holderObj(holderLS, AnalyticCollisionObject<T, dim>::STICKY);
...
TV leftSphereCenter(2, 2, 1.95);
TV rightSphereCenter(2, 2, 2.05);
T sphereRadius = 0.05;

auto leftSphereTransform = [](T time, AnalyticCollisionObject<T, dim> &object) {
  T speed = 0.1;
  TV translation_velocity(-1 * speed, 0, 0);
  TV translation(-1 * speed * time, 0,
...
AnalyticCollisionObject<T, dim>
    leftObject(leftSphereTransform, leftSphere,
               AnalyticCollisionObject<T, dim>::STICKY);
AnalyticCollisionObject<T, dim>
    rightObject(rightSphereTransform, rightSphere,
                AnalyticCollisionObject<T, dim>::STICKY);
...
//     TV translation = TV(xVel * (time - startTime), yVel * (time - startTime),
//                         zVel * (time - startTime));
...
// Sphere<T, dim> sphere(sphereCenter1, sphereRadius);
// AnalyticCollisionObject<T, dim>
//     sphereObject(sphereTransform, sphere,
//                  AnalyticCollisionObject<T, dim>::STICKY);
...
// AnalyticCollisionObject<T, dim> wallObj(holderLS, AnalyticCollisionObject<T,
// dim>::STICKY); init_helper.addAnalyticCollisionObject(wallObj);
...
//    TV translation(0, 1 * speed * time, 0); //multiply each velocity by dt to
//    get dx! object.setTranslation(translation, translation_velocity);
//};
//
// Sphere<T, dim> sphere(sphereCenter, sphereRadius);
// AnalyticCollisionObject<T, dim> sphereObject(sphereTransform, sphere,
// AnalyticCollisionObject<T, dim>::SEPARATE); sphereObject.setFriction(0.9);
// //need friction to try and separate layers!
// init_helper.addAnalyticCollisionObject(sphereObject);
