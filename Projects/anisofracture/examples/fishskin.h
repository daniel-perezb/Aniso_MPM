...
helper_output = "output/fleece_50k";
...
if (1) {
  ...
  std::string filename = "TetMesh/fleece.mesh";
  ...
  if (1) {
    ...
    std::function<bool(TV, int)> inflow = [](TV X, int vertex) {
      return (X - TV(-1.1574, -0.05, 0.01)).norm() < 0.06;
    };
    std::function<bool(TV, int)> outflow = [](TV X, int vertex) {
      return (X - TV(0.56, 0.16, 0.05)).norm() < 0.06;
    };
    StdVector<TV> tet_wise_fiber;
    fiberGen(samples, indices, inflow, outflow, tet_wise_fiber,
             node_wise_fiber);
  }
  ...
  T residual_stress = .1;
  particles_handle.transform(
      [&](int index, Ref<T> mass, TV &X, TV &V) { X += TV(2, 2, 2); });
...
{
  T radius = 0.14;
  Sphere<T, dim> leftLS(TV(0, 0, 0) + TV(-0.497, -0.36, 0.13) + TV(2, 2, 2),
                        radius);
  AnalyticCollisionObject<T, dim> leftObject(
      leftLS, AnalyticCollisionObject<T, dim>::STICKY);
  init_helper.addAnalyticCollisionObject(leftObject);
}
{
  T radius = 0.2;
  Sphere<T, dim> leftLS(TV(0, 0, 0) + TV(0.063, -0.36, 0.13) + TV(2, 2, 2),
                        radius);
  AnalyticCollisionObject<T, dim> leftObject(
      leftLS, AnalyticCollisionObject<T, dim>::STICKY);
  init_helper.addAnalyticCollisionObject(leftObject);
...
{
  T radius = 0.2;
  Sphere<T, dim> leftLS(TV(0, 0, 0) + TV(-0.937, -0.36, 0.13) + TV(2, 2, 2),
                        radius);
  AnalyticCollisionObject<T, dim> leftObject(
      leftLS, AnalyticCollisionObject<T, dim>::STICKY);
  init_helper.addAnalyticCollisionObject(leftObject);
