...
// model.scaleFiberStiffness(0, 10); //does nothing if isotropic is set in the
// model as true
...
AxisAlignedAnalyticBox<T, dim> sphere(TV(2.32, 0.93, 1.83),
                                      TV(2.42, 1.13, 2.13));
AnalyticCollisionObject<T, dim>
    leftObject(leftTransform, sphere, AnalyticCollisionObject<T, dim>::STICKY);
