// Hanging Torus -- hang toruses that get more and more heavy to show off
// inextensibility and large time steps
...
std::string filename =
    "TetMesh/torus90k.mesh"; // 50k is plenty for nice dynamics and no debris
std::string filename2 = "TetMesh/ropes20k.mesh";
MpmParticleHandleBase<T, dim> particles_handle_torus =
    init_helper.sampleFromTetWildFile(filename, rhoTorus);
MpmParticleHandleBase<T, dim> particles_handle_ropes =
    init_helper.sampleFromTetWildFile(filename2, rhoRopes);
...
// MpmParticleHandleBase<T, dim> particles_handle_torus =
// init_helper.sampleInAnalyticLevelSet(torusLS, rhoTorus, ppc);
// CappedCylinder<T, dim> cylinder1(0.03, 0.6, Vector<T, 4>(1, 0, 0, 0),
// TV(2.4, 2.35, 2)); MpmParticleHandleBase<T, dim> particles_handle_ropes =
// init_helper.sampleInAnalyticLevelSet(cylinder1, rhoRopes, 8);
...
if (inextensible) { // inextensible
  std::string path(
      "output/3D_HangingTorus/3D_HangingTorus_Inextensible_torusYoungs" +
      E_str + "_torusRho" + rho_str + "_percent" + percent_str + "_eta" +
      eta_str + "_dx" + dx_str + "_residual" + res_str);
  sim.output_dir.path = path;
} else { // anisotropic elasticity
  std::string path(
      "output/3D_HangingTorus/3D_HangingTorus_AnisoElasticity_torusYoungs" +
      E_str + "_torusRho" + rho_str + "_percent" + percent_str + "_eta" +
      eta_str + "_dx" + dx_str + "_fiberScale" + scale_str + "_residual" +
      res_str);
  sim.output_dir.path = path;
}
...
if (1) {
  ...
  sim.output_dir.createPath();
  std::string vtk_path2 = DataDir().path + "/../Projects/anisofracture/" +
...
if (!inextensible) {
  modelRopes.setExtraFiberStiffness(
      0,
      Python::fiberScale); // only scale elasticity if anisotropic elasticity!
}
particles_handle_torus.addFBasedMpmForceWithAnisotropicPhaseField(
    a_0, alphas, percentage, l0, modelTorus, eta, zeta, allow_damage,
    Python::residual);
particles_handle_ropes.addFBasedMpmForceWithAnisotropicPhaseField(
    a_0, alphas, percentage, l0, modelRopes, eta, zeta, allow_damage,
    Python::residual);
...
AnalyticCollisionObject<T, dim>
    ground_object(ground_ls, AnalyticCollisionObject<T, dim>::STICKY);
...
AnalyticCollisionObject<T, dim>
    ceiling_object(ceiling_ls, AnalyticCollisionObject<T, dim>::STICKY);
